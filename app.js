...
var fs = require('fs')
var logger = fs.createWriteStream('report_Rebeccatass.txt', {
...
function BOMBA(tweet) {
	...
	if ( tweet.user.screen_name !== 'rebeccatass' ) {
		if( testoTweet.includes('RT ') == false && inRisposta === null || testoTweet.includes('RT @'+tweet.user.screen_name) == true && inRisposta === null){
			...
			var r = Math.round((Math.random() * 4 + 1) *600000 + 20*60000); //r = Math.floor(Math.random() * 4) + 1;
